...
describe('redashClient', () => {
  ...
  describe('fetchQueryById', () => {
    ...
    it('should handle no response from server error', async () => {
      // Test the case where request was sent but no response received (line 157-158)
      // This simulates network timeout or connection reset
      nock(REDASH_URL).get('/api/queries/456').replyWithError('Network timeout');

      await expect(fetchQueryById(456)).rejects.toThrow();
    });
  });
  ...
  describe('updateQuery', () => {
    ...
    it('should handle 404 errors', async () => {
      ...
      await expect(updateQuery(999, { name: 'Test' })).rejects.toThrow(
        'Failed to update query 999'
      );
    });
  });
  ...
  describe('fetchQueryTags', () => {
    ...
    it('should handle non-array response format', async () => {
      // Test the case where response.data is not an array (line 315)
      nock(REDASH_URL)
  ...
  describe('Module initialization', () => {
    it('should throw error when environment variables are missing', async () => {
      ...
      // Try to import the module, which should throw during loadConfiguration()
      await expect(async () => {
        await import('../../src/redashClient.js?' + Date.now()); // Query param to bust cache
      }).rejects.toThrow('REDASH_URL and REDASH_API_KEY must be provided');
